...
if [ ! -f .env ]; then
  cp .env.example .env
  tput setaf 3; echo "No .env found — created one from .env.example. Edit it and set strong passwords before continuing."
fi
...
echo "For Mac and Windows, refer to the official guide https://suricatoos.wiki"
echo "#########################################################################"

echo " "
tput setaf 4;
echo "Installing Suricatoos and its dependencies"

echo " "
if [ "$EUID" -ne 0 ]
  then
  tput setaf 1; echo "Error installing Suricatoos, Please run this script as root!"
...
tput setaf 4;
echo "#########################################################################"
echo "Installing Suricatoos"
echo "#########################################################################"
make certs && make build && make up && tput setaf 2 && echo "Suricatoos is installed!!!" && failed=0 || failed=1
...
if [ "${failed}" -eq 0 ]; then
  ...
  tput setaf 2 && printf "\n%s\n" "Thank you for installing Suricatoos, happy recon!!"
  echo "In case you have unapplied migrations (see above in red), run 'make migrate'"
else
  tput setaf 1 && printf "\n%s\n" "Suricatoos installation failed!!"
fi
