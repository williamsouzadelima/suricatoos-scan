...
function check_suricatoos_update() {
  if (
  ...
  Promise.all([
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        window.open("https://www.suricatoos.wiki/update", "_blank");
      }
    });
  });
}
...
function showAfterUpdatePopup() {
  ...
  const currentVersion = document.body.getAttribute("data-suricatoos-version");
  ...
  if (lastShownVersion !== currentVersion) {
    ...
    // Swal.fire({
    //   title: isFirstRun ? "Welcome to Suricatoos!" : "Thanks for updating!",
    ...
    // }).then((result) => {
    //   if (result.isConfirmed) {
    //     window.open("https://suricatoos.wiki/changelog/latest", "_blank");
    //   }
